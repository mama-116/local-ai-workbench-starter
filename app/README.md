...
Ollamaを既定にし、LAN上のvLLMも選べるWindows向けFletデスクトップチャットです。会話は端末内またはプライベートLAN内の推論先へ送信し、クラウドAPIや月額サービスは使いません。vLLMのAPIキーは環境変数だけから読みます。
...
- LAN上のvLLM接続先 `http://192.168.1.17:18080` とモデルの切替
...
初期接続先はこのPCの `http://127.0.0.1:11434`、LAN端末の `http://192.168.1.17:11434`、LAN上のvLLM `http://192.168.1.17:18080` です。公開IP、ホスト名、HTTPS、認証情報付きURLは登録できません。vLLMはOpenAI互換の `/v1` APIを内部で利用します。

vLLMを選ぶ前に、APIキーをアプリ起動プロセスへ渡してください。キーはリポジトリ、接続設定JSON、SQLite、会話本文へ保存しません。

```powershell
$env:LOCAL_LLM_CHAT_VLLM_API_KEY = '<vLLM API key>'
