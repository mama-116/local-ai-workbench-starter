...
## Profile・関係表示（2026-07-26）

採用済みのシーン・キャスト画面へ、全正式キャストの好感度常時表示、選択人物の関係、信頼、緊張、「AIが思う関係」、変化理由を追加する操作を比較した。いずれも数値を利用者の善悪評価に使わず、ハート、経験値、報酬演出を使わない。

| 案 | 中心となる操作 | 向いている人 | 採用理由 | 強い反論 |
|---|---|---|---|---|
| A: キャストチップ型 | 上部の全キャラクターを一覧確認 | 会話への没入を優先する人 | 既存画面への変更が最小で、全員の好感度を常時確認できる | 数値の理由や関係履歴が分かりにくい |
| B: 関係パネル型 | 上部で全員を確認し、右欄で選択人物を詳しく見る | 信頼形成と説明可能性を重視する人 | 好感度、信頼、緊張、AIの関係認識、変化理由を同時に確認できる | 右欄が混み、正史記憶が下へ押される |
| C: 関係トレイ型 | 入力前に対象人物と関係状態を確認 | 自分の発言と関係変化のつながりを意識したい人 | 行動と関係変化の因果を理解しやすい | 常に点数を意識させ、会話がゲーム的になりやすい |

推奨・採用はB。上部キャストチップには1〜5人の好感度を常時表示し、右側パネルは選択人物だけを詳しく表示する。場面、正史記憶、秘密、確認待ちは右欄下部で折りたたみ可能にする。狭幅では右欄をドロワーへ移すが、選択人物の好感度はヘッダーへ残す。

- [A案モック](mockups/relationship-a-cast-chips.png)
- [B案モック（採用）](mockups/relationship-b-panel.png)
- [C案モック](mockups/relationship-c-tray.png)

見直し条件は、関係詳細の利用率20%未満、会話欄の狭さに関する具体的意見が3件以上、または好感度を上げることだけを目的にした代表入力が20%を超えた場合とする。詳細なデータ境界と撤退条件は [Profile・世界線・関係形成 設計書](RELATIONSHIP_PROFILE_DESIGN.md) を参照する。
