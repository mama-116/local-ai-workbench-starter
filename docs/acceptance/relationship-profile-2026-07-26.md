...
- 好感度の実変化時だけ、常時チップへミント／アンバーの外周と符号付き差分を一時表示
...
| 全pytest | 457成功、1 skip。skipは既存のWindows symlink権限条件 |
| distribution pytest | 7成功 |
| `mypy --strict src tests` | 成功、154 source files |
...
- 専用一時DBで「反映」後にミント外周と `+1`、Undo後にアンバーの下降情報が表示され、約0.9秒後に通常表示へ戻る
...
局所好感度エフェクト追加後の実画面レビューではCritical 1件（Fletのクリックイベントが候補ID用の既定引数を上書きし、関係候補の反映時に画面が停止する）を検出した。Fletイベントと候補IDを分離するhandler factory、人物チップの同種修正、回帰試験を追加し、専用一時DBで反映・Undo・時間経過後の復帰を再確認した。
