...
- LAN上の認証付きvLLM OpenAI互換Provider（`192.168.1.17:18080`）のモデル一覧、逐次応答、停止、観測。APIキーは `LOCAL_LLM_CHAT_VLLM_API_KEY` のプロセス環境変数だけから読み、サイズ不明モデルを推測表示しない
- 会話、キャラクター版、モデル設定、発言、分岐、実行状態の永続化
- 発言書き直しと回答再生成で元を残す分岐
- 会話一覧のメニューから行う1〜120文字の会話名変更、ゴミ箱移動と復元、確認したゴミ箱内全会話の原子的な完全削除、非active・非root分岐の非表示と復元
- 起動時の中断応答復旧
...
- 通常会話の保存後処理から、既存の安全分類済み記憶候補を世界線Profileへ追記し、関係候補をloopback Ollamaの厳格Schemaで抽出する経路。関係値は右側パネルで利用者が反映するまで変化せず、却下とUndoができ、候補処理失敗は保存済み会話を巻き戻さない
- OllamaのLAN接続が応答途中またはモデル一覧取得中に切れた場合、通信詳細を露出せず専用接続エラーへ変換する契約。生成本体は自動再送せず、読取り専用のモデル一覧・モデル詳細だけは古いkeep-alive切断時に1回再接続する
- vLLMのOpenAI互換 `/v1/models` と `/v1/chat/completions` を実機接続し、`deepseek-v4-flash-2bit` の一覧取得、`1+1=2` の逐次応答、入力・出力トークン取得を確認。ツール呼出しは未対応として通常会話へフォールバックし、Group/構造化出力の実機受入は未実施

## 検証済み

- [S-01〜S-09受入記録](acceptance/S01-S09-2026-07-17.md)（独立展開したPhase 0診断と各基準の試験手順・実績）
- `pytest`: 501件成功（Windows固有条件により1件skip）
- `mypy --strict`: 165ファイル、エラー0件
...
- WindowsポータブルZIP 71.45 MiB、2663エントリ、グループ会話・単独チャット長期記憶・正史記憶を含むアプリEXEと `LocalNotesMCP.exe` をロック済み本番依存で同梱、禁止ファイル0件、SHA-256 `f055d6fc79cca7f0943f377a56877535161a3635ed969e3877449ee5ebc8309c` 一致
- 配布版 `LocalLLMChat.exe` が `PYTHONPATH` の手動設定なしで20秒以内に初期化readyへ到達し、検査終了後に対象プロセスだけを停止
