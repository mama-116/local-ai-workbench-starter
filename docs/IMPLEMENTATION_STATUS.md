...
- 用途別モデル設定から記憶抽出用のこのPC／確認済みLAN接続と文章生成モデルを動的選択し、接続ID・endpoint fingerprint・モデルdigestを保存時と実行直前に再検証する経路
- 記憶抽出設定が未保存、モデル消失、接続・構造化出力に失敗した場合も、登録済み明示表現だけを最大8件まで決定論的に補完する経路
- 食べ物の好みを商品名・状態条件へ複数ターンで段階更新し、全ユーザー出典を追記保持する経路。対象が複数なら確認待ち、対象のない状態語は無保存
- 各完了済みユーザー発言の「覚えておいて」から、現在分岐の根拠を最大8件選択し、原文一致する保存文と、発言時の参加者を自動選択した登録Characterチェック一覧を確認して、1人以上の明示的な知識範囲へ冪等保存する補助UI
