...
1イベントの変化量上限、初期値、時間減衰は未決とする。固定試験で、性格差を表現しつつ同じイベントの再処理で値が変わらない範囲を実測して決める。
...
Profile管理専用画面のUIは未決である。編集、共有範囲、履歴、利用停止、完全削除を含む理由の異なる3案を比較してから実装する。
...
| RP-D08 | 未決 | 指標の初期値、1イベントの変化上限、時間減衰 | 利用者 | 性格差を含む固定シナリオの期待値を定義した時点 |
| RP-D09 | 未決 | Profile管理専用画面 | 利用者 | 理由の異なる3案とモックを比較した時点 |
