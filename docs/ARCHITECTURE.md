...
開発用の直接起動は `app/.local-data` を会話・設定・監査データの固定保存先とする。通常のWindowsポータブル版はFletの利用者データ領域を使う。この開発PC専用のmerge済み最新版ランチャーは例外として、元の開発worktreeの `app/.local-data` を `LOCAL_LLM_CHAT_DATA_DIR` で子プロセスだけへ渡し、直接起動と同じデータを継続利用する。ランチャーはSQLiteや関連ファイルをコピー、移動、削除しない。
...
明示記憶は型付き正史記憶とは別の追記台帳とする。単独会話の完了済み利用者発言から確認ダイアログを開き、発言全文を初期値として1〜200文字へ編集した内容だけを同期的に保存する。「覚えて」などの自然言語から保存要求を自動判定しない。ダイアログを開くたびに新しい `request_id` を発行し、同じダイアログからの再送は同じ保存結果を返す。再度開いたダイアログは新しい記憶として扱う。
