...
UIはApplication層のCoordinatorと監査読取Serviceだけを呼び、OS操作ProviderやSQLiteを直接呼ばない。既存 `ToolCoordinator` の予算・監査パターンは再利用するが、画面操作を読み取り専用ツールへ偽装しない。
...
観測、待機、結果検証はCoordinatorの内部処理とし、モデルが無制限に `sleep` や再撮影を要求できない。初回は最大5操作、60秒、同時実行1件、承認待ち30秒とする。上限到達時は失敗として停止し、勝手に再試行しない。
...
| 決定 | 通過前に確認すること | 決定者 |
|---|---|---|
| Windows入力イベントを本物のデスクトップへ送る | Fake Providerの拒否・停止試験、計画承認、対象再検査が通る | 利用者 |
...
| CU-D-09 | 未決 | 承認画面と実行中画面のレイアウト | 利用者 | 理由の異なる3案とモック比較後 |
...
| 2 | 観測専用スパイク | スクリーンショットとUI Automationからメモ帳編集領域を10条件で取得できる | 画面を読むだけで変更しない |
| 3 | 承認付き実OS Provider | 1計画承認後だけメモ帳へ入力し、5回連続成功する | 保存なしの試作として使える |
| 4 | 初版受入 | 停止・競合・強制終了を含め、異なる条件で10回連続成功する | メモ帳固定プロファイルとして使える |
